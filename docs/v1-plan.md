...
- [ ] 登录 / 登出 / 会话过期处理（REST 401 与 WS 4001 都回登录页）
- [ ] bot 列表、选择、在线状态显示；活跃 bot 记忆跨重启
- [ ] 歌词窗口：置顶 / 透明 / 无边框 / 不进任务栏 / 可拖动 / 位置记忆
- [ ] 锁定（鼠标穿透）/ 解锁（控制条出现、可拖动）
- [ ] 样式：字号、字体、粗细、原色、高亮色、透明度、描边 —— 即改即见
- [ ] 偏移：±0.5s 步进、重置、持久化、仅影响歌词不影响锚点
- [ ] 双行显示；暂停冻结；切歌 <1s 换词；他端 seek ≤2s 收敛；无歌词占位
- [ ] 全部设置（服务器、样式、偏移、位置、活跃 bot）持久化
